...
def load_data_and_init_factors(config):
    ...
    matrix_u, matrix_v = init_latent_factors(
        num_users, num_items, config.num_factors, rng_key_factors
    )
...
def uv_factorization_dense_um(
    mat_u, mat_v, mat_um, num_epochs=1, learning_rate=0.01, reg_param=0.1
):
        ...
        for i in range(mat_um.shape[0]):
            for j in range(mat_um.shape[1]):
                ...
                for k in range(mat_u.shape[1]):
                    mat_u = mat_u.at[i, k].add(
                        learning_rate * (eij * mat_v[k, j] - reg_param * mat_u[i, k])
                    )
                    mat_v = mat_v.at[k, j].add(
                        learning_rate * (eij * mat_u[i, k] - reg_param * mat_v[k, j])
                    )
    return mat_u, mat_v


# jit-compile the function, pointing out the static arguments
uv_factorization_dense_um = jax.jit(
    uv_factorization_dense_um,
...
def uv_factorization_tf_slow(mat_u, mat_v, train_ds, config):
    ...
    def update_uv_for_record(config, mat_u, mat_v, record):
        ...
        for k in range(mat_u.shape[1]):
            mat_u = mat_u.at[i, k].add(
                lr * (abs_error_ij * mat_v[k, j] - rp * mat_u[i, k])
            )
            mat_v = mat_v.at[k, j].add(
                lr * (abs_error_ij * mat_u[i, k] - rp * mat_v[k, j])
            )
        return mat_u, mat_v

    for epoch in range(config.num_epochs):
        print(f"In uv_factorization_tf_slow, starting epoch {epoch}")
        for record in tfds.as_numpy(
            train_ds.batch(config.batch_size_training).prefetch(
...
def uv_factorization_vec_no_reg(mat_u, mat_v, train_ds, valid_ds, config):
    ...
    def update_uv(mat_u, mat_v, record, lr):
        loss_value, grad = jax.value_and_grad(mse_loss_one_batch, argnums=[0, 1])(
            mat_u, mat_v, record
        )
    ...
    for epoch in range(config.num_epochs):
        ...
        valid_loss = mse_loss_all_batches(
            mat_u, mat_v, valid_ds, config.batch_size_predict_with_mse
        )
...
class Flags:
    ...
    evaluate_uv_factorization_reg = False
...
if __name__ == "__main__":
    ...
    if Flags.evaluate_uv_factorization_dense_um:
        ...
        matrix_u, matrix_v = init_latent_factors(
            num_users, num_items, num_factors, rng_key_factors
        )
        ratings = jax.random.randint(
            rng_key_r, shape=(num_users, num_items), minval=1, maxval=10
        )
        matrix_u, matrix_v = uv_factorization_dense_um(
            matrix_u, matrix_v, ratings, num_epochs=num_epochs
        )
    ...
    if Flags.evaluate_uv_factorization_tf_slow:
        ratings_tf, matrix_u, matrix_v, num_users, num_items = (
            load_data_and_init_factors(config)
        )
        train_ds, valid_ds, test_ds = data.split_train_valid_test_tf(ratings_tf, config)

        # Dummy predictions, with random factors
        predictions_and_targets = predict_and_compare(
            matrix_u, matrix_v, test_ds, config
        )
        ...
        matrix_u, matrix_v = uv_factorization_tf_slow(
            matrix_u, matrix_v, train_ds, config
        )
        predictions_and_targets = predict_and_compare(
            matrix_u, matrix_v, test_ds, config
        )
    ...
    if Flags.evaluate_uv_factorization_vec_no_reg:
        ...
        def show_metrics_and_examples(message, matrix_u, matrix_v):
            print(message)
            mse_all_batches = mse_loss_all_batches(
                matrix_u, matrix_v, test_ds, config.batch_size_predict_with_mse
            )
        ...
        show_metrics_and_examples(
            "====== Before optimization =====", matrix_u, matrix_v
        )

        # Optimize the factors fast
        matrix_u, matrix_v = uv_factorization_vec_no_reg(
            matrix_u, matrix_v, train_ds, valid_ds, config
        )

        show_metrics_and_examples("====== After optimization =====", matrix_u, matrix_v)

    if Flags.evaluate_uv_factorization_reg:
        ratings_tf, matrix_u, matrix_v, num_users, num_items = (
            load_data_and_init_factors(config)
        )
        train_ds, valid_ds, test_ds = data.split_train_valid_test_tf(ratings_tf, config)
        ...
        def show_metrics_and_examples(message, matrix_u, matrix_v):
            print(message)
            mse_all_batches = total_loss_all_batches(
                matrix_u, matrix_v, test_ds, config.batch_size_predict_with_mse
            )
            print("MSE examples from predict_with_mse on test_ds")
            print(mse_all_batches[: config.num_predictions_to_show])
        ...
        # Optimize the factors fast
        matrix_u, matrix_v = uv_factorization_reg(
            matrix_u, matrix_v, train_ds, valid_ds, config
        )
